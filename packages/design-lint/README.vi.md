...
## Bắt đầu nhanh theo framework (React, Vue, Svelte)

Một app dạng component được lint theo ba lớp — nối những lớp mà stack của bạn dùng:

- **Component** (`.jsx` / `.tsx` / `.vue` / `.svelte`) — **plugin ESLint** (ở trên) bắt hai "tell" chuyển được
  ngay trong editor lẫn CI; CLI bắt y hệt (glob mặc định của nó đã gồm các đuôi này).
- **Stylesheet & khối `<style>` trong SFC** — **plugin Stylelint** (ở trên) chạy toàn bộ bộ rule họ CSS.
- **HTML đã build / tĩnh** — trỏ CLI vào thư mục build: `npx norma-design-lint ./dist` (hoặc `./build` /
  `./out`). Lớp này chạy các rule a11y cấu trúc DOM — landmark, label, thứ tự heading — mà file component
  không diễn tả được. Truyền chính thư mục, **không phải** glob `dist/**`: glob rộng cố ý bỏ qua `dist`/`build`
  (để một `**/*.html` lỡ tay không lint output build cũ).

**React / Next.js** — dùng rule ESLint từ mục [ESLint](#dùng-bên-trong-eslint) cho component, cộng một script
CLI cho CSS và output đã build:
...
**Vue / Svelte** — plugin Stylelint lint các khối `<style>`; trỏ `customSyntax` của Stylelint tới định dạng
SFC của bạn (ví dụ `postcss-html`), rồi chạy CLI trên component và bất kỳ HTML đã build nào.

**Pre-commit** — chỉ lint thứ bạn đang commit, với [lint-staged](https://github.com/lint-staged/lint-staged):
...
Kích hoạt từ hook `pre-commit` sẵn có (husky hoặc simple-git-hooks). Đang dùng GitHub Actions?
`npx norma-design-lint init` tạo sẵn một workflow.
